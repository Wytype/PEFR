...
class Attention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        if mask is not None:
            ...
            x = self.proj_drop(x)
            return x
            # mask = self.qkv(mask).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
            # q = qkv[0]
            # k, v = mask[1], mask[2]
            # attn = (q @ k.transpose(-2, -1)) * self.scale
            # attn = attn.softmax(dim=-1)
...
class PatchEmbed_overlap(nn.Module):
    ...
    def forward(self, x):
        ...
        assert H == self.img_size[0] and W == self.img_size[1], \
            f"Input image size ({H}*{W}) doesn't match model ({self.img_size[0]}*{self.img_size[1]})."
        x = self.proj(x) #[128,768,14,14]

        x = x.flatten(2).transpose(1, 2) # [64, 8, 768] #[128,196,768]
        return x
...
class TransReID(nn.Module):
    ...
    def forward_features(self, x, mask, flag, camera_id, view_id):
        B = x.shape[0]
        x = self.patch_embed(x)
        mask = self.generate_patch_mask_max(mask,16)
        cls_tokens = self.cls_token.expand(B, -1, -1)  # stole cls_tokens impl from Phil Wang, thanks
        mask_token = torch.ones(B,1,device=mask.device)
        x = torch.cat((cls_tokens, x), dim=1)
        mask = torch.cat((mask_token, mask), dim=1)
        ...
        if flag:
            ...
            #         x_cross = blk(x, mask)
            #         x_ori = blk(x)
            #         x = (1-a)*x_ori + a*x_cross
            #     else:
            #         x = blk(x)
            return x

    def forward(self, x, mask, flag=False, cam_label=None, view_label=None):
        x = self.forward_features(x, mask, flag, cam_label, view_label)

        return x



    def generate_patch_mask_max(self,raw_mask, patch_size):
        B, C, H, W = raw_mask.shape
        mask = raw_mask.squeeze(1)  # [B, H, W]
        mask = mask.view(B,
                         H // patch_size, patch_size,
                         W // patch_size, patch_size)  # [B, H/p, p, W/p, p]

        # 取每个patch的最大值 (dim2和dim4是patch内部空间维度)
        patch_mask = mask.amax(dim=(2, 4))  # [B, H/p, W/p]

        # 展平为序列格式
        patch_mask = patch_mask.view(B, -1)  # [B, num_patches]

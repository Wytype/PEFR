...
class build_vit_pefr_224(nn.Module):
    def __init__(self, clas_nums, camera_num, view_num, cfg, factory):
        super(build_vit_pefr_224, self).__init__()
        model_path = cfg.MODEL.PRETRAIN_PATH
        pretrain_choice = cfg.MODEL.PRETRAIN_CHOICE
        self.cos_layer = cfg.MODEL.COS_LAYER
        self.neck = cfg.MODEL.NECK
    ...
    def forward(self, x, mask, flag=True, occ_agu=False, personOCC_pro=0.5, label=None, cam_label=None, view_label=None):  # label is unused if self.cos_layer == 'no'
        # import time
        ...
        if occ_agu and self.training:
            ...
            if pro > (1 - personOCC_pro):
                # 用另一副图的从上而下去做！模拟行人遮挡
                x_ori = x.clone()
                mask_ori = mask.clone() #B,1,H,W

                # #优化标签
                row_has_one = torch.any(mask_ori == 1, dim=-1).squeeze(-1)
                row_has_one = row_has_one.squeeze(1)
                row_has_one = row_has_one.to(torch.uint8) #(B,H)
                # # 使用 torch.argmax 找到第一个 1 的索引
                # first_one = torch.argmax(row_has_one, dim=1)
                # 为了找到最后一个 1 的索引，我们可以翻转数组，然后再次使用 torch.argmax
                # 翻转数组
                row_has_one_flipped = torch.flip(row_has_one, dims=[1])
                # 找到最后一个 1 的索引（在翻转后的数组中）
                last_one_flipped = torch.argmax(row_has_one_flipped, dim=1)

                # 计算最后一个 1 的实际索引
                last_one = x.size(2) - 1 - last_one_flipped
                #
                part_L = int(side_H / 7)  # 分成7部分每部分像素大小
                #
                last_L = torch.div(last_one, part_L, rounding_mode='floor')
                occ_label_ori = torch.zeros(x.size(0), 7, dtype=torch.float32).to('cuda')
                batch_indices = torch.arange(x.size(0), dtype=torch.long).to('cuda')
                for i in range(7):
                    occ_label_ori[batch_indices, i] = (last_L >= i).to(torch.float32)


                occ_label_ori = torch.transpose(occ_label_ori, 0, 1)


                # occ_label_ori = torch.ones((7, x.size(0))).to('cuda') #7,B

                from_copy_tensor_label = torch.ones(7).to('cuda') #7,
                ...
                index = np.array(range(int(x.size(0)))) #[64,]index:[0~63]
                index = np.roll(index, int(x.size(0) / 2)) #[32~63 0~31]
                a_index = index[:int(x.size(0) / 2)] #[32~63]
                b_index = index[int(x.size(0) / 2):] #[0~31]
                b_index = np.roll(b_index, random.randint(1, int(x.size(0) / 2)))
                a_index = np.roll(a_index, random.randint(1, int(x.size(0) / 2)))
                b = np.concatenate([b_index[0::4], b_index[1::4], b_index[2::4], b_index[3::4]])
                a = np.concatenate([a_index[0::4], a_index[1::4], a_index[2::4], a_index[3::4]])
                ...
                copy_tensor = torch.zeros_like(x) #复制x大小全0
                rand_index = torch.tensor(index).to('cuda')
                copy_tensor[:, :, :, :] = x[rand_index] #赋值

                h_index = random.randint(3, 6)
                h_start = h_index * part_L
                h_end = side_H #从hstart往下

                x[:, :, h_start:h_end, :] = copy_tensor[:, :, 0:h_end - h_start, :] #拿上半部分去填充到x的下半部分
                mask[:, :, h_start:h_end, :] = 0
                from_copy_tensor_label[0:int((h_end - h_start) / part_L)] = 0     #上半部分为0下半部分为1即下半部分遮挡

                occ_info = torch.zeros(side_H) #224,
                occ_info[h_start:h_end] = 1
                occ_info = occ_info.chunk(7) #tensortuple:7,
                # print(occ_info)
                occ_label = torch.zeros(7)
                for i in range(7):
                    occ_label[i] = torch.sum(occ_info[i]) / part_L #遮挡概率
                occ_label = occ_label.unsqueeze(dim=1).repeat((1, x.size(0))).to('cuda')  # （7,B）
                ...
                x = torch.cat([x_ori, x], dim=0)
                mask = torch.cat([mask_ori, mask], dim=0)
            else:
                # 用别人的部分去roll到1/2下去做！
                '''之前的生成乱序的方法
                index = np.array(range(int(x.size(0))))
        ...
        # mask_first = mask[4].cpu().numpy()
        # img_first = x[4].cpu().numpy()
        # body_first = np.transpose(mask_first, (1, 2, 0))
        # img_first = np.transpose(img_first, (1, 2, 0))
        # plt.imshow(body_first[:, :, :])
        # plt.axis('off')
        # plt.show()
        ...
        #     mask4test = mask.clone()
        #     row_has_one = torch.any(mask4test == 1, dim=-1).squeeze(-1)
        #     row_has_one = row_has_one.squeeze(1)
        #     row_has_one = row_has_one.to(torch.uint8)  # (B,H)
        #     # # 使用 torch.argmax 找到第一个 1 的索引
        #     first_one = torch.argmax(row_has_one, dim=1)
        ...
        #     part_L = int(side_H / 7)  # 分成7部分每部分像素大小
        #
        #     first_L = torch.div(first_one, part_L, rounding_mode='floor')
        #     last_L = torch.div(last_one, part_L, rounding_mode='floor')
        #     occ_label_ori = torch.zeros(x.size(0), 7, dtype=torch.float32).to('cuda')
        #     batch_indices = torch.arange(x.size(0), dtype=torch.long).to('cuda')
        #     for i in range(7):
        #         condition = (last_L > i) & (i >= first_L)
        #         occ_label_ori[batch_indices, i] = condition.to(torch.float32)
        #     occ_similar = torch.transpose(occ_label_ori, 0, 1)


        # mask = mask.expand(-1, 3, -1, -1)
        # mask = x * mask_expanded
        # features = self.base(x, cam_label=cam_label, view_label=view_label)
        features = self.base(x, mask, flag, cam_label=cam_label, view_label=view_label)

        b1_feat = self.b1(features)  # [64, 129, 768] [128,169,768]
        # global_feat = b1_feat[:, 0]
        ...
        # 这里分离了occ_pre的梯度！
        feat_map_cls = einops.rearrange(feat_map_cls, 'b (h w) l -> b l h w', h=14)# [128, 768, 14, 12]
        # global_feat = self.gap(feat_map_cls).squeeze()
        # feat_map = feat_map_cls.detach()  # (B,C,L)

        global_feat = self.gap(feat_map_cls).squeeze(dim=3)#[128, 768, 1]
        global_feat = global_feat.squeeze(dim=2)#[128,768]
       
        feat_map = feat_map_cls.detach()

        h_feat = self.hen(feat_map).squeeze(dim=-1)  # B, C, 7 #[128,768,7]
        h_local_0 = h_feat[..., 0]
        h_local_1 = h_feat[..., 1]
        h_local_2 = h_feat[..., 2]
        h_local_3 = h_feat[..., 3]
...
class build_vit_pefr_256(nn.Module):
    def __init__(self, clas_nums, camera_num, view_num, cfg, factory):
        super(build_vit_pefr_256, self).__init__()
    ...
    def forward(self, x, mask, flag=True, occ_agu=False, personOCC_pro=0.5, label=None, cam_label=None,
...
def make_model(cfg, num_class, camera_num, view_num):
    ...
    elif cfg.MODEL.NAME == 'vit_pefr':
        if cfg.INPUT.SIZE_TRAIN == [224, 224]:
            model = build_vit_pefr_224(num_class, camera_num, view_num, cfg,__factory_T_type)
        else:
            model = build_vit_pefr_256(num_class, camera_num, view_num, cfg, __factory_T_type)

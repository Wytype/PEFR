...
def do_train(cfg,
    ...
    for epoch in range(1, epochs + 1):
        flag = True if epoch > 100 else False
        # flag = True
        ...
        model.train()
        for n_iter, (body, img, vid, target_cam, target_view) in enumerate(train_loader):
            optimizer.zero_grad()
            ...
            #检测数据顺序有没有对齐
            # mask_first = mask[0].cpu().numpy()
            # mask_first = np.transpose(mask_first, (1, 2, 0))
            # plt.imshow(mask_first[:, :, :])
            # plt.axis('off')
            # plt.show()
            # body_first = body[0].cpu().numpy()
            # img_first = img[0].cpu().numpy()
            # body_first = np.transpose(body_first, (1, 2, 0))
...
def do_inference(cfg,
    ...
    img_path_list = []
    flag=True
    for n_iter, (body, img, pid, camid, camids, target_view, imgpath) in enumerate(val_loader):
        with torch.no_grad():

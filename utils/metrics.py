...
def eval_func(distmat, q_pids, g_pids, q_camids, g_camids, max_rank=50):
    ...
    for q_idx in range(num_q):
        ...
        if len(np.unique(q_camids)) == 1 and len(np.unique(g_camids)) == 1:
...
class R1_mAP_eval():
    ...
    def update_w(self, output):  # called once for each batch
        ...
        # self.occ_comolabel.append(occ_sim.cpu())
    ...
    def compute_loop_featurerecover(self, k_num=5, first_dist='global', recover_method='hard_label', n_num=3):
        ...
        q_3 = q_recovery[:n_num,:,:]
        q_w3 = q_recovery_w[:n_num,:]
        q_recoveryloop1 = q_recovery.clone()
        q_recovery_wloop1 = q_recovery_w.clone()

        q_recovery = torch.cat((q_3,q_lf[n_num:,:,:]),dim=0)
        q_recovery_w = torch.cat((q_w3, q_vis[n_num:,:]),dim=0)

        for i in range(n_num, 7):
            if self.reranking:
                print('=> Enter reranking')
            ...
            g_lf_Knearst = g_lf[:, indices_feat_recovery[:, :k_num], :]  # (7,2210,10,1024)
            g_lf_Knearst = g_lf_Knearst.permute(1, 2, 0, 3)  # (2210, 10,7,1024)

            if recover_method == 'hard_label':
                g_w_Knearst = g_w[:, indices_feat_recovery[:, :k_num]]
            elif recover_method == 'soft_label':
                g_w_Knearst = g_vis[:, indices_feat_recovery[:, :k_num]]
            ...
            #     q_need_vis = (1 - q_vis).permute(1, 0)  # (2210,7)
            #     q_vis_ori = q_vis.permute(1, 0)
            #     q_recovery = q_lf.permute(1, 0, 2) * q_vis_ori.unsqueeze(dim=-1) + g_near_feat * q_need_vis.unsqueeze(
            #         dim=-1)
            #
            #     q_recovery = q_recovery.permute(1, 0, 2)
            #     # q_recovery_w = torch.ones_like(q_vis)
        ...
        for q_idx in range(num_q):
            ...
            if ((indice_remove_ori[0] != indice_remove_recov[0]) and (
                    g_pids[indice_remove_ori[0]] != g_pids[indice_remove_recov[0]])):
    ...
    def compute_featRecovery123(self,k_num=5,first_dist = 'global', recover_method='hard_label'):  # called after each epoch
        ...
        if self.reranking:
            ...
            distmat_global = euclidean_distance(q_gf, g_qf)
            distmat_local = euclidean_distance_weight(q_lf, g_lf, q_w, g_w)
            # (2210, 17661)
        ...
        """Evaluation with market1501 metric
               ...
               """
        max_rank = 50
        num_q, num_g = distmat_global.shape
        # distmat g
        #    q    1 3 2 4
        #         4 1 2 3
        if num_g < max_rank:
            max_rank = num_g
            print("Note: number of gallery samples is quite small, got {}".format(num_g))
        indices_global = np.argsort(distmat_global, axis=1)
        indices_local = np.argsort(distmat_local, axis=1)
        #  0 2 1 3
        #  1 2 3 0
        # g_pids[indices] 相当于gallary中的pid，按相似度最前排序，行id为行人的id，(q_num,g_num)
        # q_pids[:, np.newaxis] (q_num,1)
        # matches(q_num,g_num)
        indices_global = torch.tensor(indices_global)
        indices_local = torch.tensor(indices_local)

        g_lf_Knearst_global = g_lf[:, indices_global[:, :k_num], :]  # (7,2210,10,1024)
        g_lf_Knearst_global = g_lf_Knearst_global.permute(1, 2, 0, 3)  # (2210, 10,7,1024)
        g_lf_Knearst_local = g_lf[:, indices_local[:, :k_num], :]  # (7,2210,10,1024)
        g_lf_Knearst_local = g_lf_Knearst_local.permute(1, 2, 0, 3)  # (2210, 10,7,1024)

        if recover_method == 'hard_label':
            g_w_Knearst_global = g_w[:, indices_global[:, :k_num]]
            g_w_Knearst_local = g_w[:, indices_local[:, :k_num]]
        elif recover_method == 'soft_label':
            g_w_Knearst_global = g_vis[:, indices_global[:, :k_num]]
            g_w_Knearst_local = g_w[:, indices_local[:, :k_num]]

        # g_w_Knearst = g_w[:, indices[:, :k_num]]  #  (7,2210,10)
        g_w_Knearst_global = g_w_Knearst_global.permute(1, 2, 0).unsqueeze(dim=-1)  # (2210,10,7,1)
        g_w_Knearst_represnet_global = g_w_Knearst_global.squeeze(dim=-1).max(dim=1)[0]
        g_near_feat_global = ((g_lf_Knearst_global * g_w_Knearst_global)).sum(dim=1) / g_w_Knearst_global.sum(dim=1)  # (2210, 7,1024)

        g_w_Knearst_local = g_w_Knearst_local.permute(1, 2, 0).unsqueeze(dim=-1)  # (2210,10,7,1)
        g_w_Knearst_represnet_local = g_w_Knearst_local.squeeze(dim=-1).max(dim=1)[0]
        g_near_feat_local = ((g_lf_Knearst_local * g_w_Knearst_local)).sum(dim=1) / g_w_Knearst_local.sum(
            dim=1)  # (2210, 7,1024)

        a=0.5
        fusion_feat = a*g_near_feat_global+(1-a)*g_near_feat_local
        fusion_w = a*g_w_Knearst_represnet_global+(1-a)*g_w_Knearst_represnet_local

        if recover_method == 'hard_label':
            # q_w (7,2210)
            q_need_w = (1-q_w).permute(1,0) #(2210,7)
            q_w_ori = q_w.permute(1, 0)
            q_recovery = q_lf.permute(1,0,2) *q_w_ori.unsqueeze(dim=-1) + g_near_feat_global* q_need_w.unsqueeze(dim=-1)
            q_recovery = q_recovery.permute(1,0,2)
            # q_recovery_w = torch.ones_like(q_w)
            q_recovery_w = q_w + q_need_w.permute(1, 0) * g_w_Knearst_represnet_global.permute(1, 0)
        elif recover_method == 'soft_label':
            q_need_vis = (1 - q_vis).permute(1, 0)  # (2210,7)
            q_vis_ori = q_vis.permute(1, 0)
            q_recovery = q_lf.permute(1, 0, 2) * q_vis_ori.unsqueeze(dim=-1) + fusion_feat * q_need_vis.unsqueeze(dim=-1)

            q_recovery = q_recovery.permute(1, 0, 2)
            # q_recovery_w = torch.ones_like(q_vis)
            q_recovery_w = q_vis + q_need_vis.permute(1, 0) * fusion_w.permute(1, 0)
        ...
        for q_idx in range(num_q):
            ...
            # remove gallery samples that have the same pid and camid with query
            order_ori = indices_local[q_idx]  # select one row
            # if q_pid == 72 and q_camid==0:  #camid要减1
            #     print(g_pids[order_ori][:10])
            #     print("okk")
            remove_ori = (g_pids[order_ori] == q_pid) & (g_camids[order_ori] == q_camid)
            ...
            indice_remove_ori = indices_local[q_idx][keep_ori]
            indice_remove_recov = indices_feat_recovery[q_idx][keep_recov]
    ...
    def compute_featRecovery_comolabel(self, k_num=5, first_dist='global', recover_method='hard_label'):  # called after each epoch

        g_feats = torch.cat(self.g_feats, dim=0)  # [N,L]
        l_feats = torch.cat(self.l_feats, dim=1)  # [7, N, L]
        feats_w = torch.cat(self.weight, dim=1)  # [7, N]
        feats_visibility = torch.cat(self.weight_w, dim=1) #[7,N]
        comolabel = torch.cat(self.occ_comolabel, dim=1)

        condition = (comolabel == 1) & (feats_visibility <= 0.5)

        # 使用 torch.where 实现条件替换
        feats_visibility = torch.where(condition, torch.tensor(0.5), feats_visibility)
        ...
        q_recovery = torch.cat((q_3, q_lf[3:, :, :]), dim=0)
        q_recovery_w = torch.cat((q_w3, q_vis[3:, :]), dim=0)
        ...
        for i in range(3, 7):
            ...
            if recover_method == 'hard_label':
                ...
                q_need_vis = 1 - q_recovery_w[i]  # (2210,)
                q_vis_ori = q_recovery_w[i]  # (2210,)
                q_recovery_layer = q_recovery[i] * q_vis_ori.unsqueeze(dim=-1) + g_near_feat[:, i,
                                                                                 :] * q_need_vis.unsqueeze(
                    dim=-1)  # (2210, 1024)
                q_recovery[i] = q_recovery_layer  # 更新 q_recovery 的第 i 层
                q_recovery_w[i] = q_recovery_w[i] + q_need_vis * g_w_Knearst_represnet.permute(1, 0)[
                    i]  # 更新 q_recovery_w 的第 i 层
